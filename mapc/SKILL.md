...
> 在当前项目目录下运行 `mkmap` 生成骨架，然后按规范补全 MINDMAP.md。
> 触发方式：`/mapc`
...
1. 运行 `mkmap` 生成骨架 MINDMAP.md
2. 读取生成的 MINDMAP.md
3. 按下方规范，分 Session 补全所有节点的"作用 / 输入 / 输出"以及类描述、文件描述
4. 用户输入数字（如 `1`）推进到下一个 Session
...
> **前置条件**：仓库根目录已存在由 `mkmap` 自动生成的骨架 MINDMAP.md，其中包含目录 / 文件 / 类 / 函数节点。你需要在此骨架上逐步补全"作用 / 输入 / 输出"以及类描述、文件描述。
